"""
scraper/stores/ivory.py — Product catalog scraper for Ivory (ivory.co.il).

Ivory is an Israeli consumer-electronics and home-appliance retailer with
physical stores across Israel and an online shop at ivory.co.il.

Strategy:
    1. Fetch ``/sitemap.xml`` using :meth:`~scraper.base.BaseScraper.get_static_html`
       (Ivory does NOT use Shopify).
    ...
    4. Extract product data using BeautifulSoup, prioritising:
           ...
           b. Ivory-specific CSS selectors (``.product-title``, ``.price``,
              ``[data-price]``, ``[itemprop]`` microdata).
    5. Return ``[item.model_dump() for item in items]``.

Concurrency:
    Up to 5 product pages are fetched in parallel via
    ``asyncio.Semaphore(5)`` to avoid hammering ivory.co.il.

Usage::

    async with IvoryScraper() as scraper:
...
#: Root URL of the Ivory online store.
IVORY_BASE_URL = "https://www.ivory.co.il"
...
# CSS selectors likely to exist on Ivory product pages.
# TODO: verify and update after inspecting live ivory.co.il markup.
_TITLE_SELECTORS = [
    ".product-title",
    ".product_title",
    "[class*='product-title']",
...
class IvoryScraper(BaseScraper):
    """
    Product catalog scraper for Ivory (ivory.co.il).

    Ivory is a large Israeli electronics and home-appliance retailer that
    does NOT use Shopify; therefore :meth:`detect_shopify` always returns
    ``False`` and the scraper falls back to sitemap discovery + Playwright
    page rendering.
    ...
    def __init__(
        ...
        super().__init__(store_url=IVORY_BASE_URL, headless=headless)
    ...
    async def scrape(self) -> list[dict]:
        """
        Scrape the Ivory product catalog and return normalised product dicts.

        Flow:
            1. Discover product URLs from the Ivory sitemap.
        ...
        """
        logger.info("IvoryScraper.scrape() starting for %s", self.store_url)

        product_urls = await self._discover_product_urls()
        if not product_urls:
            logger.warning("No product URLs found for Ivory — returning empty list")
            return []

        product_urls = product_urls[: self.max_products]
        logger.info("Ivory: scraping %d product pages", len(product_urls))
        ...
        logger.info("IvoryScraper finished: %d products scraped", len(items))
    ...
    async def _discover_product_urls(self) -> list[str]:
        """
        Probe known Ivory sitemap paths and return a deduplicated list of
        product page URLs.
        ...
        for path in _SITEMAP_PATHS:
            sitemap_url = urljoin(IVORY_BASE_URL + "/", path.lstrip("/"))
            logger.debug("Ivory: probing sitemap at %s", sitemap_url)
            try:
                xml = await self.get_static_html(sitemap_url)
                urls = self._parse_sitemap(xml)
                if urls:
                    logger.info(
                        "Ivory: found %d product URLs in %s", len(urls), sitemap_url
                    )
                    return urls
            except Exception as exc:
                logger.debug("Ivory sitemap probe failed for %s: %s", sitemap_url, exc)
                continue

        logger.warning("Ivory: all sitemap probes failed — no product URLs found")
        return []

    def _parse_sitemap(self, xml: str) -> list[str]:
        """
        Parse a sitemap XML string and extract URLs that look like Ivory
        product pages.

        Ivory product URLs typically contain ``/product/``, ``/item/``,
        or match numeric-slug patterns like ``/online/p-<id>``.
    ...
    def _is_product_url(url: str) -> bool:
        """
        Heuristic: return ``True`` if the URL looks like an Ivory product page.

        Matches paths containing ``/product``, ``/item``, ``/online/``,
        or a numeric-slug pattern common on ivory.co.il.
        ...
        Note:
            TODO: Inspect live ivory.co.il sitemap to confirm URL patterns
            and refine this heuristic if needed.
        """
        # TODO: verify against live ivory.co.il sitemap structure
        patterns = [
            r"/product",
            r"/item",
            r"/p/",
    ...
    async def _scrape_product_page(self, url: str) -> Optional[ProductItem]:
        """
        Load an Ivory product page with Playwright and extract product data.

        Extraction priority:
            1. Schema.org ``Product`` JSON-LD (most reliable).
            2. Ivory-specific CSS selectors / ``itemprop`` microdata.
        ...
        try:
            ...
            logger.debug("Ivory: failed to fetch %s: %s", url, exc)
            return None
        ...
        # Strategy 2: Ivory CSS selectors + itemprop microdata
    ...
    def _extract_css(
        ...
        """
        Extract product data using Ivory-specific CSS selectors and
        ``itemprop`` microdata as a fallback.

        Tries each selector in the known-Ivory selector lists and returns
        the first matching value for name, price, availability, and image.
        ...
        Note:
            TODO: Validate selectors against live ivory.co.il pages and
            refine as needed — CSS classes may change with site redesigns.
        """
        # --- Name ---
        name: str = ""
        for selector in _TITLE_SELECTORS:
            ...
            if el:
                ...
                if name:
                    break
        if not name:
            logger.debug("Ivory CSS: no name found on %s", page_url)
            return None
        ...
        for selector in _AVAILABILITY_SELECTORS:
            ...
            if el:
                ...
                if (
                    "instock" in text_lower
                    or "במלאי" in text_lower
                    or "in stock" in text_lower
                ):
                    availability = True
                    break
                if (
                    "outofstock" in text_lower
                    or "אזל" in text_lower
                    or "out of stock" in text_lower
                    or "אין במלאי" in text_lower
                ):
                    availability = False
                    break
